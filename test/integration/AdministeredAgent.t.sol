...
contract AdministeredAgent_IntegrationTests is Test {
    ...
    AdministeredAgent internal administeredActor;

    address internal admin = makeAddr("admin");
    address internal actor = makeAddr("actor");

    function setUp() external {
        administeredActor = new AdministeredAgent(admin);
    ...
    function test_call_withValue() external {
        ...
        assertEq(address(administeredActor).balance, 0);
        assertEq(target.balance,                     0);

        vm.prank(actor);
        bytes memory result = administeredActor.call{ value: 0.5 ether }(target, abi.encodeWithSelector(MockTarget.foo.selector, 1));
    ...
    function test_batchCall_withValue() external {
        ...
        vm.prank(actor);
        bytes[] memory results = administeredActor.batchCall{ value: 0.9 ether }(targets, data, values);
        ...
        assertEq(address(administeredActor).balance, 0.2 ether);
    ...
    function test_sendValue() external {
        ...
        assertEq(address(administeredActor).balance, 1 ether);
        assertEq(target.balance,                     0);

        vm.prank(actor);
        administeredActor.sendValue(target, 0.4 ether);

        assertEq(address(administeredActor).balance, 0.6 ether);
